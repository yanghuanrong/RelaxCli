    ...
    <div class="meta" ref="meta">
      <div class="highlight">
        ...
        <div class="hljs-footbar" v-if="fileList.length">
          <code class="hljs">
            <div class="hljs-problems__tips">
              Problems <span>{{ fileList.length }}</span>
            </div>
            <div
              v-for="(item, i) in fileList"
              class="hljs-problems__list"
              :key="'file' + i"
            >
              <svg
                width="20"
                ...
                viewBox="0 0 24 24"
                stroke="currentColor"
                class="flex-none text-amber-400"
              >
                <path
              ...
              <p>
                ...
                <span
                  class="hljs-string download"
                  :title="`download ${item.name}.${item.ext}`"
                  @click="download(item)"
                  >{{ item.name }}.{{ item.ext }}</span
                >
              </p>
            </div>
          </code>
